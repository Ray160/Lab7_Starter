...
const router = new Router(function () {
  ...
  let recipeCards = document.getElementsByClassName("section--recipe-cards")[0];
  let expand = document.getElementsByClassName("section--recipe-expand")[0];
...
function createRecipeCards() {
  ...
  recipes.forEach(recipe => {
      ...
      //if(recipeData[recipe]['page-name']){
...
function bindPopstate() {
  ...
  window.onpopstate = function() {
    if(document.location.hash.substring(1)){
      router.navigate(document.location.hash.substring(1), true);
    }else{
